...
typedef struct {   // All information about a specific stored pokemon
  ...
  uint8_t shake_state;  // 0=0 when being captured, 1=1 "" "", 2=2 "" "", 3=3 "" "", >=4 => ignore this value
} poke_info;
...
typedef struct {   // All information about a specific stored pokemon
  const char name[8];   // **NOTE; Lots of runtime errors when trying this as 'const char name[XX]' due to initialisation problems -> will only work like this on the stack in my experience
  const uint8_t name_length;
...
typedef struct {
  ...
  const uint8_t category; // Which type of item this is e.g. 0=>NULL, 1=>Ball, 2=>Heal, ...
} poke_item;
